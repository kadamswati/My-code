...
let _defaultParams = {
    ...
    colorScheme: null,
    innerRadiusOfDonutChart: 35,
    outerRadiusOfDonutChart: 100
};

class DonutPlot {
    /**
    * Adds a Donut plot with default or custom viewer settings.
    ...
    * 2. Apply custom background, label, selection, outline colors to donut plot.
    * 3. Apply inner and outer radius to donut plot.
    * 4. Apply highlight opacity to donut plot.
    * @param {{ backgroundColor: any,  selectionColor: any,  labelColor: any, highlightOpacity: number, innerRadiusOfDonutChart: number,  outerRadiusOfDonutChart: number}} initParams Pass the customized settings to apply on 2D plot instace. If customization is not passed then default settings will apply on 2D plot instance
    * @returns {Object} Returns added donut plot.
    */
    constructor(initParams) {
        ...
        this.innerRadiusOfDonutChart = params.innerRadiusOfDonutChart;
        this.outerRadiusOfDonutChart = params.outerRadiusOfDonutChart;
        this.canvas = null;
        this.name = null;
    ...
    */
    ...
    /**
    * Add the donut plot.
    * @param {any} newData The newData parameter take the data for creation of Donut plot.
    * @param {number} idForPlotCreation The idForPlotCreation parameter is the id for each data object.
    * @param {string} keyForPlotCreation The keyForPlotCreation parameter is the key for the creation of Donut plot.
    * @param {string} layoutType The layoutType parameter is used for checking the data types.
    * @returns {Object} It returns an object for acknowledgement of this API to the consumer side.
    */
    addDonutPlot(newData, idForPlotCreation, keyForPlotCreation, layoutType, idKeyValue = undefined) {
        try {
            let arc = d3.arc()
                .innerRadius(this.innerRadiusOfDonutChart)
                .outerRadius(this.outerRadiusOfDonutChart);

            let dataArr = [];
            // Create the donut plot layout for number or count layout type
            if (layoutType == 'value') {
                if (keyForPlotCreation == 'string' || keyForPlotCreation == 'bool') {
                    error.textContent = "Failed to add donut plot for string using number layout type"
                }
                else {
            ...
            // Create the donut plot layout for string, bool and date datatype
            else {
            ...
            this.group.append("path")
                .attr("d", arc)
                .attr("fill", function (d, i) { return arcColor(i); })
                .attr("stroke", this.outlineColor) // outline for each region
            ...
            let returnValue = {
                "status": "failed",
                "message": "Failed to add donut plot." + err.message
            }
            return returnValue;
        }
    }
    ...
    /**
    * Select the elements of the donut plot.
    * @param {number} elementIDs The elementIDs parameter takes an ids for selecting the elements from Donut plot.
    * @returns {Object} It returns an object for acknowledgement of this API to the consumer side.
    */
    ...
    /**
    * Highlight the elements of the donut plot.
    * @param {number} elementIDs The elementIDs parameter takes an ids for highlighting the elements from Donut plot.
    * @returns {Object} It returns an object for acknowledgement of this API to the consumer side.
    */
    ...
    /**
    * Clean the instance of the donut plot.
    */

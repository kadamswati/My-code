...
class LinePlot {
    /**
    * Adds a Line plot with default or custom viewer settings.
    ...
    * 2. Apply custom background, axis, selection, lineplot, dot, gridline colors to line plot.
    * 3. Apply highlight opacity to line plot.
    * @param {{ backgroundColor: any,  selectionColor: any,  axisColor: any, highlightOpacity: number, linePlotColor: any,  dotColor: any, gridLineColor: any}} initParams Pass the customized settings to apply on 2D plot instace. If customization is not passed then default settings will apply on 2D plot instance
    * @returns {Object} Returns added line plot.
    */
    ...
    */
    ...
    /**
    * Add the line plot.
    * @param {any} newData The newData parameter take the data for creation of Line plot.
    * @param {number} idForPlotCreation The idForPlotCreation parameter is the id for each data object.
    * @param {string} keyForLinePlotCreation The keyForLinePlotCreation parameter is the key for the creation of Line plot.
    * @param {string} valueForLinePlotCreation The valueForLinePlotCreation parameter is the value for the creation of Line plot.
    * @param {boolean} gridLines The gridLines parameter is used to add the grid lines into the line plot.
    * @param {string} layoutType The layoutType parameter is used for checking the data types.
    * @returns {Object} It returns an object for acknowledgement of this API to the consumer side.
    */
    addLinePlot(newData, idForLinePlotCreation, valueForLinePlotCreation, keyForLinePlotCreation, layoutType, gridLines = { x: false, y: false }, idKeyValue = undefined) {
        try {
            let dataArr = [];
            const numberOfTicks = { x: 15, y: 30 }

            // Create the line plot layout for number or count layout type
            if (layoutType == 'value') {
                if (valueForLinePlotCreation == 'string' || valueForLinePlotCreation == 'bool') {
                    error.textContent = 'Failed to add bar plot for string and boolean using number layout type'
                }

                else {
                    let response = utilsInstance.filterLineAndScatterData(newData, idForLinePlotCreation, valueForLinePlotCreation, keyForLinePlotCreation, idKeyValue);
                    if (response.status == 'success') {
                        dataArr = response.data;
                    }
                }
            }
            // Create the line plot layout for string, bool and date datatype
            else {
                let response = utilsInstance.filterOtherTypeData(newData, idForLinePlotCreation, valueForLinePlotCreation, idKeyValue);
                if (response.status == 'success') {
                    dataArr = response.data;
                }
            }
            ...
            if (layoutType == 'value') {
                ...
                this.svg
                    ...
                    .attr("d", d3.line()
                        .x(function (d) { return x(d.value) })
                        .y(function (d) { return y(d.name) + y.bandwidth() / 2 })
                    )
            ...
            let returnValue = {
                'status': 'failed',
                'message': 'Failed to add Line plot.' + err.message
            }
            return returnValue;
        }
    }
    ...
    /**
    * Select the elements of the line plot.
    * @param {number} elementIDs The elementIDs parameter takes an ids for selecting the elements from Line plot.
    * @returns {Object} It returns an object for acknowledgement of this API to the consumer side.
    */
    ...
    /**
    * Highlight the elements of the line plot.
    * @param {number} elementIDs The elementIDs parameter takes an ids for highlighting the elements from Line plot.
    * @returns {Object} It returns an object for acknowledgement of this API to the consumer side.
    */
    ...
    /**
    * Clean the instance of the line plot.
    */
